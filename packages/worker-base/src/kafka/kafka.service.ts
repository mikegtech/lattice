...
export class KafkaService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    @Inject('KAFKA_OPTIONS') private readonly options: { expectedSchemaVersion?: string },
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        if (result.status === 'retry') {
          ...
          const delay = result.delay ?? this.config.kafka.retryBackoffMs * Math.pow(2, retryCount);
        ...
        if (retryCount < maxRetries && this.isRetryableError(lastError)) {
          const delay = this.config.kafka.retryBackoffMs * Math.pow(2, retryCount);
  ...
  async produce<T>(
    ...
    const envelopeOptions: Parameters<typeof createEnvelope<T>>[0] = {
      ...
      stage: options.stage as 'raw' | 'parse' | 'chunk' | 'embed' | 'upsert' | 'delete' | 'audit',
  ...
  private async sendToDLQ(
    ...
    try {
      const dlqPayload = {
        ...
        error_classification: this.isRetryableError(error) ? 'retryable' : 'non_retryable',
  ...
  private extractHeaders(
    message: { headers?: Record<string, Buffer | string | undefined> },
