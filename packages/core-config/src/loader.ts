...
export function loadConfig(options: ConfigLoaderOptions = {}): FullConfig {
  ...
  const baseRaw = {
    ...
    service: {
      ...
      pipeline: getEnvVar(env, { key: 'PIPELINE', default: 'mail-indexing' }, strict),
    },
    logLevel: getEnvVar(env, { key: 'LOG_LEVEL', default: 'info' }, strict),
  };

  const kafkaRaw = {
    bootstrapServers: getEnvVar(env, { key: 'KAFKA_BOOTSTRAP_SERVERS', required: true }, strict),
    securityProtocol: getEnvVar(env, { key: 'KAFKA_SECURITY_PROTOCOL', default: 'SASL_SSL' }, strict),
    ...
    retryBackoffMs: getEnvVar(env, { key: 'KAFKA_RETRY_BACKOFF_MS', default: 100, transform: toInt }, strict),
  };

  const postgresRaw = {
    host: getEnvVar(env, { key: 'POSTGRES_HOST', default: 'localhost' }, strict),
    port: getEnvVar(env, { key: 'POSTGRES_PORT', default: 5432, transform: toInt }, strict),
    database: getEnvVar(env, { key: 'POSTGRES_DB', default: 'lattice' }, strict),
    user: getEnvVar(env, { key: 'POSTGRES_USER', default: 'lattice' }, strict),
    password: getEnvVar(env, { key: 'POSTGRES_PASSWORD', required: true }, strict),
    ssl: getEnvVar(env, { key: 'POSTGRES_SSL', default: false, transform: toBool }, strict),
    poolMin: getEnvVar(env, { key: 'POSTGRES_POOL_MIN', default: 2, transform: toInt }, strict),
    poolMax: getEnvVar(env, { key: 'POSTGRES_POOL_MAX', default: 10, transform: toInt }, strict),
    connectionTimeout: getEnvVar(env, { key: 'POSTGRES_CONNECTION_TIMEOUT', default: 10000, transform: toInt }, strict),
  };
  ...
  const datadogRaw = {
    ...
    runtimeMetricsEnabled: getEnvVar(env, { key: 'DD_RUNTIME_METRICS_ENABLED', default: true, transform: toBool }, strict),
  };

  const validateSchema = <T>(schema: ZodSchema<T>, data: unknown, name: string): T => {
