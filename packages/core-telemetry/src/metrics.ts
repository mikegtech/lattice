...
import { filterMetricTags, type MetricTags } from './tags.js';
...
class DatadogMetrics implements Metrics {
  ...
  increment(name: string, value = 1, tags?: Record<string, string>): void {
    if (this.dogstatsd) {
      this.dogstatsd.increment(this.formatName(name), value, this.formatTags(tags));
    }
  }
  ...
  histogram(name: string, value: number, tags?: Record<string, string>): void {
    if (this.dogstatsd) {
      this.dogstatsd.histogram(this.formatName(name), value, this.formatTags(tags));
    }
  }
