...
import type { LogTags, ServiceTags } from "./tags.js";
...
class PinoLogger implements Logger {
	...
	child(bindings: Record<string, unknown>): Logger {
		...
		return new PinoLogger(this.pino.child(redactedBindings), this.baseTags);
	}
}

export function createLogger(options: LoggerOptions): Logger {
	validateTags(
		options.serviceTags as unknown as Record<string, string | undefined>,
		"logger",
	);
	...
	const pinoInstance = (
		pino as unknown as (opts: pino.LoggerOptions) => pino.Logger
	)(pinoOptions);
