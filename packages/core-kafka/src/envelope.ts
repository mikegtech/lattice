...
const VALID_STAGES = ['raw', 'parse', 'chunk', 'embed', 'upsert', 'delete', 'audit'];
...
export function validateEnvelope(
  ...
  // Validate domain
  if (envelope['domain'] && !VALID_DOMAINS.includes(envelope['domain'] as string)) {
    errors.push(`Invalid domain: ${envelope['domain']}`);
  }

  // Validate stage
  if (envelope['stage'] && !VALID_STAGES.includes(envelope['stage'] as string)) {
  ...
  if (envelope['schema_version'] && typeof envelope['schema_version'] === 'string') {
    if (!/^v\d+$/.test(envelope['schema_version'])) {
      errors.push(`Invalid schema_version format: ${envelope['schema_version']}`);
    }
  }
