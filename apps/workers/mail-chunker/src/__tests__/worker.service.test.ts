...
describe('MailChunkerService', () => {
  const createPayload = (overrides: Partial<MailParsePayload> = {}): MailParsePayload => ({
...
async function simulateProcess(
  ...
  context: { traceId: string; topic: string; partition: number; offset: string },
) {
  try {
    ...
    if (chunksExist) {
      return { status: 'skip' as const, reason: 'Chunks already exist for this version' };
    }

    // Fetch content
    const emailContent = await mockChunkRepository.getEmailContent(payload.email_id);
    ...
    const attachments = await mockChunkRepository.getAttachmentContents(payload.email_id);
    ...
    const chunks = mockChunkingService.chunkEmail({
      ...
      attachments: attachments.map((a: { attachment_id: string; extracted_text: string }) => ({
