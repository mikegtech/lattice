...
export class ChunkingService {
	...
	chunkEmail(input: ChunkInput): GeneratedChunk[] {
		...
		if (input.subject && input.subject.trim().length > 10) {
			const subjectChunk = this.createSubjectChunk(
				input.emailId,
				input.subject,
			);
			if (subjectChunk) {
		...
		this.logger.debug("Chunking complete", {
			...
			attachment_chunks: allChunks.filter((c) => c.sourceType === "attachment")
				.length,
		});
	...
	private chunkText(
		...
		while (remaining.length > 0) {
			...
			const { chunk: chunkText, remaining: rest } =
				this.tokenizer.splitAtTokenBoundary(
					remaining,
			...
			if (chunkText.length === 0) {
				...
				if (rest.length > 0 && this.config.overlapTokens > 0) {
					const overlapChars = this.tokenizer.tokensToChars(
