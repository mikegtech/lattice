...
export class ChunkingConfig {
  ...
  constructor() {
    this.targetTokens = parseInt(process.env['CHUNK_TARGET_TOKENS'] ?? '400', 10);
    this.overlapTokens = parseInt(process.env['CHUNK_OVERLAP_TOKENS'] ?? '50', 10);
    this.maxTokens = parseInt(process.env['CHUNK_MAX_TOKENS'] ?? '512', 10);
