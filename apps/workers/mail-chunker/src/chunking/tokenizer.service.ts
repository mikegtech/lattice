...
export class TokenizerService {
   ...
   */
  splitAtTokenBoundary(text: string, targetTokens: number): { chunk: string; remaining: string } {
    ...
    if (sentenceMatch && sentenceMatch.index !== undefined) {
      breakPoint = Math.floor(targetChars * 0.8) + sentenceMatch.index + sentenceMatch[0].length;
      return {
        chunk: text.substring(0, breakPoint).trim(),
        remaining: text.substring(breakPoint).trim(),
      };
