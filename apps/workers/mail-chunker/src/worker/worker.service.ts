...
import { ChunkingService, type GeneratedChunk } from '../chunking/chunking.service.js';
import { ChunkingConfig } from '../chunking/chunking.config.js';

@Injectable()
export class MailChunkerService extends BaseWorkerService<MailParsePayload, MailChunkPayload[]> {
  constructor(
    kafka: KafkaService,
  ...
  > {
    ...
    try {
      ...
      if (chunksExist) {
        this.logger.info('Chunks already exist for this email and version, skipping', {
          ...logContext,
          chunking_version: chunkingVersion,
          normalization_version: normalizationVersion,
        });
        this.telemetry.increment('messages.skipped', 1, { stage: 'chunk', reason: 'duplicate' });
        return { status: 'skip', reason: 'Chunks already exist for this version' };
      }

      // Fetch content from Postgres for canonical source
      const emailContent = await this.chunkRepository.getEmailContent(payload.email_id);

      if (!emailContent) {
        this.logger.warn('Email not found in database', logContext);
        this.telemetry.increment('messages.error', 1, { stage: 'chunk', error: 'not_found' });
        return {
          status: 'dlq',
      ...
      const attachmentContents = await this.chunkRepository.getAttachmentContents(payload.email_id);
      ...
      const chunks = this.chunkingService.chunkEmail({
        ...
        textNormalized: emailContent.text_normalized ?? payload.body.text_normalized ?? '',
      ...
      this.telemetry.timing('chunking_time_ms', Date.now() - startChunk, { stage: 'chunk' });

      if (chunks.length === 0) {
        this.logger.info('No chunks generated (empty content)', logContext);
        this.telemetry.increment('messages.skipped', 1, { stage: 'chunk', reason: 'empty' });
      ...
      this.telemetry.timing('db.insert_ms', Date.now() - startDb, { stage: 'chunk' });
      ...
      this.telemetry.increment('chunks.created', chunks.length, { stage: 'chunk' });
      ...
      this.logger.error('Failed to chunk email', err.stack, JSON.stringify(logContext));
  ...
  protected async produceOutput(
    ...
    originalMessage: { envelope: { tenant_id: string; account_id: string; domain: string } },
  ): Promise<void> {
